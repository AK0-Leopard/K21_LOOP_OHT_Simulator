...
namespace Mirle.Agvc.Simulator
{
    public partial class Form1 : Form
    {
        ...
        private void ConfigToUI()
        {
            ...
            txtLocalPort.Text = middlerConfigs.LocalPort.ToString();
        ...
        private void LoadConfigs()
        {
            ...
            int.TryParse(configHandler.GetString("Middler", "LocalPort", "5001"), out int tempPort);
        ...
        private void SetDataGridViewFromCmdNum(int selectCmdNum)
        {
            ...
            switch (cmdType)
            {
                ...
                case EnumCmdNums.Cmd31_TransferRequest:
                    ...
                    cmd31.GuideAddressStartToLoad.AddRange(new List<string>());
                    cmd31.GuideAddressToDestination.AddRange(new List<string>());
                ...
                case EnumCmdNums.Cmd35_CarrierIdRenameRequest:
                    ID_35_CARRIER_ID_RENAME_REQUEST cmd35 = new ID_35_CARRIER_ID_RENAME_REQUEST();
                    cmd35.OLDCSTID = "CA0070";
                ...
                case EnumCmdNums.Cmd135_CarrierIdRenameResponse:
                    ID_35_CARRIER_ID_RENAME_REQUEST cmd135 = new ID_35_CARRIER_ID_RENAME_REQUEST();
                    //cmd135.ReplyCode = 0;
        ...
        {
            if (dgvOht.SelectedRows.Count <= 0)
            {
                MessageBox.Show("Please Select OHT.");
                return;
            }

            string port_id = dgvOht.SelectedRows[0].Cells[1].ToString();
            if (scApp.AgentDic.ContainsKey(port_id))
            {
                scApp.AgentDic[port_id].SetAlarmToOHBC();
            }
        }
        ...
        private void btnConnect_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtIp.Text.Trim().Equals(string.Empty))
                {
                ...
                {
                    MessageBox.Show("Please Input Remote Port.");
                    return;
                }

                if (txtLocalPort.Text.Trim().Equals(string.Empty))
                {
                    MessageBox.Show("Please Input Local Port.");
                    return;
                }

                string port_id = txtPort.Text.Trim();
                if(scApp.AgentDic.ContainsKey(port_id))
                {
                    MessageBox.Show("Remote Port Already Exist.");
                    return;
                }

                MiddlerConfigs newConfig = new MiddlerConfigs();

                newConfig.ClientNum = middlerConfigs.ClientNum;
                newConfig.ClientName = middlerConfigs.ClientName;
                newConfig.LocalIp = middlerConfigs.LocalIp;
                newConfig.RecvTimeoutMs = middlerConfigs.RecvTimeoutMs;
                newConfig.SendTimeoutMs = middlerConfigs.SendTimeoutMs;
                newConfig.MaxReadSize = middlerConfigs.MaxReadSize;
                newConfig.ReconnectionIntervalMs = middlerConfigs.ReconnectionIntervalMs;
                newConfig.MaxReconnectionCount = middlerConfigs.MaxReconnectionCount;
                newConfig.RetryCount = middlerConfigs.RetryCount;
                newConfig.SleepTime = middlerConfigs.SleepTime;
                newConfig.IsServer = middlerConfigs.IsServer;
                newConfig.CycleRunIntervalMs = middlerConfigs.CycleRunIntervalMs;

                newConfig.RemoteIp = txtIp.Text;
                newConfig.RemotePort = int.Parse(txtPort.Text);
                newConfig.LocalPort = int.Parse(txtLocalPort.Text);

                MiddleAgent middleAgent = new MiddleAgent(newConfig);
                ...
                scApp.AgentDic.Add(port_id, middleAgent);
                updateDGV();
            }
            catch (Exception ex)
            {
        ...
        private void btnDisConnect_Click(object sender, EventArgs e)
        {
            ...
            string port_id = dgvOht.SelectedRows[0].Cells[1].Value.ToString();
            if(scApp.AgentDic.ContainsKey(port_id))
            {
                scApp.AgentDic[port_id].DisConnect();

                scApp.AgentDic.Remove(port_id);
                updateDGV();
            }

            scApp.deleteVhAddress(port_id);
        }
        ...
        private void cbAutoApplyReserve_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (scApp.AgentDic.ContainsKey(port_id))
            {
                scApp.AgentDic[port_id].AutoApplyReserve = cbAutoApplyReserve.Checked;
            }
        }
        ...
        public void SendNextRandomLoadunloadCmd()
        {
            ...
            else
            {
                ...
                if (scApp.AgentDic.ContainsKey(port_id))
                {
                    ...
                    scApp.AgentDic[port_id].SendCommand(31, pairs);
        ...
        private void btnIsNull_Click(object sender, EventArgs e)
        {
            ...
            if (scApp.AgentDic.ContainsKey(port_id))
            {
                var msg = $"IsNull={scApp.AgentDic[port_id].ServerClientAgent == null}";
                AppendDebugLogMsg(msg);
            }
        }
        ...
        private void btnSetReserveReplyOnce_Click(object sender, EventArgs e)
        {
            ...
            if (scApp.AgentDic.ContainsKey(port_id))
            {
                scApp.AgentDic[port_id].Send_Cmd36_ReservePass(txtReserveSectionId.Text, cbReserveSuccess.Checked);
            }
        }
        ...
        private void btnSet_Click(object sender, EventArgs e)
        {
            ...
            if (scApp.AgentDic.ContainsKey(port_id))
            {
                ...
                {
                    scApp.AgentDic[port_id].OperatypeSet(EnumOperaType.NormalComplete);
                }
                else if (radioButton_Abnormal_BcrReadFail.Checked == true)
                {
                    scApp.AgentDic[port_id].OperatypeSet(EnumOperaType.Abnormal_BcrReadFail);
                }
                else if (radioButton_Abnormal_BcrMisMatch.Checked == true)
                {
                    scApp.AgentDic[port_id].OperatypeSet(EnumOperaType.Abnormal_BcrMismatch);
                }
                else if (radioButton_Abnormal_BcrDuplicate.Checked == true)
                {
                    scApp.AgentDic[port_id].OperatypeSet(EnumOperaType.Abnormal_BcrDuplicate);
                }
                else if (radioButton_Abnormal_DoubleStorage.Checked == true)
                {
                    scApp.AgentDic[port_id].OperatypeSet(EnumOperaType.Abnormal_DoubleStorage);
                }
                else if (radioButton_Abnormal_EmptyRetrieval.Checked == true)
                {
                    scApp.AgentDic[port_id].OperatypeSet(EnumOperaType.Abnormal_EmptyRetrieval);
                }
                else if (radioButton_Cancel.Checked == true)
                {
                    scApp.AgentDic[port_id].OperatypeSet(EnumOperaType.CancelComplete);
                }
                else if (radioButton_Abort.Checked == true)
                {
                    scApp.AgentDic[port_id].OperatypeSet(EnumOperaType.AbortComplete);
                }
                else if (radioButton_InterlockError.Checked == true)
                {
                    scApp.AgentDic[port_id].OperatypeSet(EnumOperaType.InterlockError);
                }
            }
        }
        ...
        private void updateDGV()
        {
            ...
            List<MiddleAgent> agentLst = scApp.AgentDic.Values.ToList();

            foreach (MiddleAgent agent in agentLst)
            {
                string[] row = { agent.RemoteIP(), agent.RemotePort().ToString(), agent.LocalPort().ToString() };
