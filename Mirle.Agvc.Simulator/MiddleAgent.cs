...
namespace Mirle.Agvc.Simulator
{
    public class MiddleAgent
    {
        ...
        public MiddleAgent(MiddlerConfigs middlerConfigs)
        {
            ...
            vehicle = new Vehicle();
            operaObject = new Opera(this);

            vehicle.Remote_Port = RemotePort().ToString();
            ...
            scApp.updateVhAddress(RemotePort().ToString(), theVehicleInfo.CurrentAdrID);
            ...
            try
            {
                ...
                switch (cmdType)
                {
                    case EnumCmdNums.Cmd31_TransferRequest:
                        {
                            ...
                            AddRangeToGuideList(aCmd.GuideSectionsStartToLoad, pairs["GuideSectionsStartToLoad"]);
                    ...
                    case EnumCmdNums.Cmd35_CarrierIdRenameRequest:
                        {
                            ID_35_CARRIER_ID_RENAME_REQUEST aCmd = new ID_35_CARRIER_ID_RENAME_REQUEST();
                            aCmd.NEWCSTID = pairs["NEWCSTID"];
                            aCmd.OLDCSTID = pairs["OLDCSTID"];

                            wrappers.ID = WrapperMessage.CARRIERIDRenameReqFieldNumber;
                            wrappers.CARRIERIDRenameReq = aCmd;
                    ...
                    case EnumCmdNums.Cmd51_AvoidRequest:
                        {
                            ...
                            AddRangeToGuideList(aCmd.GuideAddresses, pairs["GuideAddresses"]);
                            aCmd.DestinationAdr = pairs["DestinationAdr"];
        ...
        public void Send_Cmd143_StatusResponse(ushort seqNum)
        {
            ...
            double VhXAxis = string.IsNullOrEmpty(blockData.XAxis) ? 0: Convert.ToDouble(blockData.XAxis);
            double VhYAxis = string.IsNullOrEmpty(blockData.YAxis) ? 0 : Convert.ToDouble(blockData.YAxis);
            double VhAngle = string.IsNullOrEmpty(blockData.Angle) ? 0 : Convert.ToDouble(blockData.Angle);
            ...
            try
            {
                ...
                iD_143_STATUS_RESPONSE.DrivingDirection = theVehicleInfo.DrivingDirection;
                ...
                iD_143_STATUS_RESPONSE.ReserveStatus = theVehicleInfo.ReserveStatus;
                ...
                iD_143_STATUS_RESPONSE.XAxis = VhXAxis;
                iD_143_STATUS_RESPONSE.YAxis = VhYAxis;
                iD_143_STATUS_RESPONSE.Angle = VhAngle;
        ...
        public void Send_Cmd36_ReservePass(string sectionId, bool isSuccess)
        {
            ...
            response.IsReserveSuccess = isSuccess ? ReserveResult.Success : ReserveResult.Unsuccess;
        ...
        private void Receive_Cmd136_TransferEventReport(object sender, TcpIpEventArgs e)
        {
            ...
            try
            {
                ...
                if (receive.EventType == EventType.ReserveReq)
                {
                    if (AutoApplyReserve)
                    {
                        response.IsReserveSuccess = ReserveResult.Success;
                    }
                    else
                    {
                        response.IsReserveSuccess = ReserveResult.Unsuccess;
                    }
                    response.ReserveInfos.Clear();
                    response.ReserveInfos.Add(receive.ReserveInfos[0]);
                }
        ...
        {
            return middlerConfigs.RemoteIp;
        }

        public int RemotePort()
        {
            return middlerConfigs.RemotePort;
        }

        public int LocalPort()
        {
            return middlerConfigs.LocalPort;
        }
